 ...
 */
 {
  ...
  class Slideshow {
    constructor(el) {
      ...
      this.DOM.titles = [
        ...this.DOM.titlesInner.querySelectorAll(".grid__item--title"),
      ];
      // The slides instances
      this.slides = [];
      [...this.DOM.el.querySelectorAll(".grid__item--slide")].forEach(
        (slide, pos) => this.slides.push(new Slide(slide, this.DOM.titles[pos]))
      );
      ...
      // Content Items
      this.DOM.contentMore = [...document.querySelectorAll('.content__item-copy-more')]
      ...
      const render = () => {
        //if ( !this.isAnimating ) {
        mouseMoveVals.translation = MathUtils.lerp(
          mouseMoveVals.translation,
          MathUtils.lineEq(-15, 15, winsize.width, 0, mousePos.x),
          0.03
        );
    ...
    setVisibleSlides() {
      ...
      this.rightSlide =
        this.slides[
          this.center + 1 <= this.slidesTotal - 1 ? this.center + 1 : 0
        ];
      this.leftSlide =
        this.slides[
          this.center - 1 >= 0 ? this.center - 1 : this.slidesTotal - 1
        ];
    ...
    calculateGap() {
      const pickAGoodSlide =
        this.center === this.slidesTotal - 1 ? this.center - 1 : this.center;
    ...
    initEvents() {
      ...
      this.clickClose = () => this.closeSlide();
      // this.DOM.contentMore.forEach(
      // (moreContent) => {
      //   moreContent.addEventListener("click", this.clickClose)
      // }
      // )
      ...
      this.DOM.interaction.center.addEventListener(
        "mouseenter",
        this.mouseenterCenterFn
      );
      ...
      this.DOM.contentItems.forEach((item) => {
        item
          .querySelector(".img-wrap--content")
          .addEventListener("click", () => this.closeSlide());
      });
    }
    navigate(direction) {
      if (this.isAnimating) {
      ...
      const upcomingPos =
        direction === "right"
          ? this.center < this.slidesTotal - 2
            ? this.center + 2
            : Math.abs(this.slidesTotal - 2 - this.center)
          : this.center >= 2
          ? this.center - 2
          : Math.abs(this.slidesTotal - 2 + this.center);
      ...
      this.center =
        direction === "right"
          ? this.center < this.slidesTotal - 1
            ? this.center + 1
            : 0
          : this.center > 0
          ? this.center - 1
      ...
      let promises = [];
      movingSlides.forEach((slide) =>
        promises.push(
          slide.move(direction === "right" ? "left" : "right", this.gap)
        )
      );
    ...
    toggleSlide(action) {
      ...
      movingSlides.forEach((slide) =>
        promises.push(
          slide[action === "open" ? "animateElementsOut" : "animateElementsIn"](
            contentItem
          )
        )
      );
          
      const previousButton = document.querySelector('.content-button')
     if (action === "open") {
  ...
  class Revealer {
    constructor(el) {
      ...
      this.DOM.el.style.height = `calc(100vw * ${Math.sin(
        (8 * Math.PI) / 180
      )} + 100vh * ${Math.cos((8 * Math.PI) / 180)})`;
    }
  }
...
const gsapLandingTitle = gsap.fromTo(".title", {opacity:0, y:"100%"} , {
  delay:1,
  opacity: 1,
  ease: "Expo.easeInOut",
  duration: 1,
...
const previousClickHandler = () => {
  gsapLanding.reverse()
  gsapLandingText.reverse()
  gsapLandingTitle.reverse()
...
const navigateHandler = (location) =>  {
    // gsapLanding.reverse()
    // gsapLandingText.reverse()
    const transitionTimeline = gsap.timeline().to(".transition-page", {
      duration: 1,
      x:"0%"
    }).to(".transition-page", {
      delay: 3,
